...
async def plot_timeseries(daily_data, company_name):
    ...
    fig, ax = plt.subplots(figsize=(4, 2.4))  # Adjust figure size for a width of 400 pixels at 300 DPI
    
    title_fontsize = 8  # Smaller size for the title
    change_fontsize = 6  # Smaller size for the change text
    value_fontsize = 5  # Smaller size for the value labels
    ...
    ax.text(0.5, 1, company_name, fontsize=title_fontsize, ha='center', va='top', transform=ax.transAxes, color='white')
    ...
    ax.text(0.5, 0.85, change_text, fontsize=change_fontsize, ha='center', va='bottom', transform=ax.transAxes, color='white')
    ...
    ax.text(0.05, first_value, f'{first_value:.2f}%', fontsize=value_fontsize, va='center', transform=ax.transData, color='white', bbox=dict(facecolor='black', alpha=0.5))
    ax.text(0.95, last_value, f'{last_value:.2f}%', fontsize=value_fontsize, va='center', transform=ax.transData, color='white', bbox=dict(facecolor='black', alpha=0.5))

    
    plt.tight_layout(pad=0.1)  # Adjust layout padding to prevent clipping of tick-labels and titles
    
    # Save the figure to a BytesIO object with higher DPI for better resolution
    image_stream = io.BytesIO()
    plt.savefig(image_stream, format='png', dpi=100, facecolor=fig.get_facecolor(), edgecolor='none')  # Higher DPI for better resolution
